...
class ControllerExtensionModuleReferralCoupon extends Controller {
  public function index($setting) {
    ...
    $replace = array(
     ...
     $customer_emails = $this->model_account_customer->checkExistCustomer();
     $data['c_email'] = $customer_emails;
    $data['error_member_existed'] = $this->language->get('error_member_existed');
  ...
  public function getReferrals() {
    ...
    // coupon
    $data['coupons'] = array();
    $querycoupon = $this->db->query("SELECT coupon_id  FROM " . DB_PREFIX . "coupon_history");
    foreach ($querycoupon->rows as $c) {
      $data['coupons'][] = array(
        'coupon_id' => $c['coupon_id'],
        // 'customer_id' => $c['customer_id'],
        // 'amount' => $c['amount'],
        // 'date_added' => date($this->language->get('date_format_short'), strtotime($c['date_added'])),
      );
      ...
      foreach ($this->request->get['referee_email'] as $key => $referee_email) {
        ...
        if ($sending_limit['remain'] > 0 && !$email_existed) {
          ...
          //
            // coupon
        // $data['coupons'] = array();
        // $querycoupon = $this->db->query("SELECT coupon_id  FROM " . DB_PREFIX . "coupon_history WHERE coupon_id = '" .  $this->db->escape($data['coupon_id']) . "'");
        // foreach ($querycoupon->rows as $c) {
        //   $data['coupons'][] = array(
        //     'coupon_id' => $c['coupon_id'],
        //   );
        ...
        // }
           ...
           //=======================NEW INSERT===========================================
           $querys = $this->db->query("SELECT referral_coupon_id FROM " . DB_PREFIX . "customer_referral_coupon WHERE coupon_id = '" . (int)$coupon_id . "'");
          ...
          $body = $this->getEmailHTML($code, $referee_name, $rc_id);


          if ( $this->validateEMAIL($referee_email) ) {
            $this->email($from, $to, $subject, $body);
          } else {
            // set post fields
            $handphone = $referee_email;
            if (strpos($handphone, '65') === false && strpos($handphone, '+65') === false) {
            ...
            $post = array(
                ...
                'appsecret' => APPSECRET,
                'receivers' => $handphone,
                'content'   => 'Hi ' . $referee_name . ', ' . $this->customer->getFirstName() . ' ' . $this->customer->getLastName() . ' would like to send you a $5 coupon code. Simply register at ' . $this->url->link('account/register') . '&ref_id=' . $this->customer->getId() .'&c_id='.$coupon_id.' See you there!'
            );
            $ch = curl_init(SMSURL);
  ...
  // public function getEmailHTML($coupon_code = '{coupon_code}', $referee_name = '{referee_name}', $referrer_message = '{referrer_message}') {
   public function getEmailHTML($coupon_code = '{coupon_code}', $referee_name = '{referee_name}',$rc_id = '{rc_id}') {
    $this->load->language('extension/module/referral_coupon');
    ...
    $register_page = $this->url->link('account/register');
    ...
    $html = str_replace(
      array('{referee_name}', '{coupon_code}', '{reward_type}', '{expire_date}', '{order_total}', '{customer_login}', '{uses_total}', '{uses_customer}', '{referrer_name}', '{store_link}', '{store_logo}', '{store_name}', '{coupon_discount}','{c_id}', '{register}','{rc_id}'),
      array($referee_name,  $coupon_code, $reward_type, $expire_date, $order_total, $customer_login, $uses_total, $uses_customer, $referrer_name, $store_link, $store_logo, $store_name, $coupon_discount, $c_id, $register_page, $rc_id),
      $this->language->get('email_body')
    );
    // $html = str_replace(
    //   array('{referee_name}', '{coupon_code}', '{reward_type}', '{expire_date}', '{order_total}', '{customer_login}', '{uses_total}', '{uses_customer}', '{referrer_name}', '{store_link}', '{store_logo}', '{store_name}', '{coupon_discount}', '{c_id}', '{register}'),
    //   array($referee_name, $coupon_code, $reward_type, $expire_date, $order_total, $customer_login, $uses_total, $uses_customer, $referrer_name, $store_link, $store_logo, $store_name, $coupon_discount, $c_id, $register_page),
    //   $this->language->get('email_body')
    // );
    return $html;
  } //getEmailHTML end
  ...
  public function updateOrder($route = '', $data = array()) {
    $order_id = $data[0];
